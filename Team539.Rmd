...
get_election_data=function(year){
  ...
  #add winner column
  dd = dd %>%
    mutate(Winner = apply(dd,1,function(x){names(which.max(x[str_detect(colnames(dd),"E$")]))}))
...
shinyApp(
  ...
  server = function(input, output, session) 
  {
    ...
    k <- reactive({
      data() %>% 
        select(`State Abbr`,`Winner`) %>%
        mutate(`Winner` = ifelse(str_detect(.$`Winner`,"Republican"),1,
                                 ifelse(
                                   str_detect(.$`Winner`,"Democratic"),2,3)))
    })
