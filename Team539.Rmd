...
While there are numerous data sources available summarizing presidential election results, none are as complete as the state by state results table on Wikipedia. Unlike other sources, these tables include state by state results for all presidential candidates, not just those from the two major parties. Therefore, using Wikipedia is not only convenient as it allows us to obtain data via web scraping from a single source for all election years, but it also gives us more refined data on the performance of third party candidates.

We generate a clean data table for each election year via scraping 
...
#alternative trial using case_when
test <- b %>% select(State,`Winner`,`electoral votes`,`Margin %`) %>%
