...
library(sf)
library(datamap)
library(rMaps)#package ‘rMaps’ is not available (for R version 3.3.1) for me-YL
...
get_election_data=function(year){
  ...
  colnames(d) = colnames(d) %>%
    ...
    str_replace("State Total|Total State|^Total$","State Abbr") %>%
...
#quick check to see which columns of output have NA's (if any)
names(apply(e,2,function(x) {any(is.na(x))}))
...
#using albersusa
usa <- usa_sf() %>% mutate(`State Abbr` = as.character(iso_3166_2))
k <- e %>% select(`State Abbr`,`Winner`,`electoral votes`,`Margin %`) %>%
  mutate(`Winner` = ifelse(str_detect(.$`Winner`,"Republican"),1,               
                           ifelse(str_detect(.$`Winner`,"Democratic"),2,3))
         )
kk <- left_join(usa,k, by ="State Abbr")
kk[is.na(kk)] <- 0
  ...
  server = function(input, output, session) 
  {
    ...
    k <- reactive({
      data() %>%
        ...
        select(`State Abbr`,`Winner`,`electoral votes`,`Margin %`)
    })
    
    kk <- reactive({
      left_join(usa,k(), by ="State Abbr") %>%
    ...
    #observe({
      #poly_pop <-tagList(
        #tags$h4(kk()$name),tags$br(),
        #sprintf("Electoral votes: %s", kk()$`electoral votes`),tags$br(),
        #sprintf("Winner: %s",kk()$`Winner`),tags$br(),
        #sprintf("Margin: %s%%",abs(kk()$`Margin %`))
        #)
    #})
    ...
    observe({
      ...
      leafletProxy("map", data = kk()) %>%
        addPolygons(smoothFactor = 0.2, fillOpacity = 0.8,
                    fillColor = ~pal(Winner),color = "black",
                    weight = 1, opacity = 1, layerId =~name, popup = kk()$name)
    })
    ...
    list_cand<-eventReactive(input$do,{
      colnames(data())%>%str_extract("[:upper:]\\w+[:space:][:upper:]\\w+")%>%na.omit()%>%unique()
    })
    observe({
      updateSelectInput(session, "cand","Candidate",
                      choices=list_cand()[1:6])
  })
    ...
    #observe({
      ...
      #pop <- input$map_shape_click
      #if(is.null(pop)){
        #return()
      #}
      
      #isolate({
        #leafletProxy("map") %>% addPopups(pop$lat,pop$lng,pop$id,
                                          #layerId = pop$id)
      #})
    #})  
  }
